...
if (isset($_GET['filtroCol']) && isset($_GET['valor'])) {
  $filas = $consultasCliente->consultarClientesFiltrados1($_GET['filtroCol'],$_GET['valor']);
}else{
...
if (isset($filas)) {    
  ...
  foreach ($filas as $fila){
    ...
    $tabla.='<tr class="limitada" scope="row">';
      ...
      $botonhab='<a href="habCli.php?NumeroIdentificacion='.$fila['NumeroIdentificacion'].'&id='.$fila['idCliente'].'"><input type="button" class="btn btn-info" value="Habilitar"></a>';

      $tabla.='<center><td class="row_buttons">'.$botonhab.'</td></center>';
    $tabla.='</tr>';
